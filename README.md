...
| `DECIMAL`         | arbitrary-precision signed decimal number of the form unscaled × 10-scale | Not supported      | `decimal` (logical)          | "fn:decimal(value, 38)", where value is a byte array containing the two's-complement representation of the unscaled integer value in big-endian byte order |
...
* `decimalFromDouble(input)`: Converts `input` to a `BigDecimal`.
